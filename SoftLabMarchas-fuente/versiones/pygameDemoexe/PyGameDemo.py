...
def depth_frame_ready(frame):
    ...
    with screen_lock:
        ...
        ctypes.memmove(address, frame.image.bits, len(address)) #memove(hasta, desde, tantos bits)
        del address
...
if __name__ == '__main__':
    ...
    def post_frame(frame):
        try:
            pygame.event.post(pygame.event.Event(KINECTEVENT, skeletons = frame.SkeletonData))# este croto me va guarando los esqueletos, y despues los imprimo con video y deph
        except:
            # event queue full
            pass

    kinect.skeleton_frame_ready += post_frame  # esto apila eventos en una cola, para ser tratados.
    
    kinect.depth_frame_ready += depth_frame_ready    
    kinect.video_frame_ready += video_frame_ready    # el evento video frame ready me actualiza el frame (en c_tyes, por referencia)
    
    kinect.video_stream.open(nui.ImageStreamType.Video, 2, nui.ImageResolution.Resolution640x480, nui.ImageType.Color)
    kinect.depth_stream.open(nui.ImageStreamType.Depth, 2, nui.ImageResolution.Resolution320x240, nui.ImageType.Depth)

    print('Controles: ')
    print('     d - ver imagen de profundidad')
    print('     v - ver video')
    print('     s - ver o no ver esqueleto')
    print('     u - incrementar angulo')
    print('     j - disminuir angulo')
